...
public class RideDAO {
	...
	public List<Ride> getAllRides() {
		StringBuilder query = new StringBuilder("SELECT r.* , u.name user_name FROM heybuddy.rides r INNER JOIN heybuddy.users u ON r.username = u.username ORDER BY time");
		return jdbcTemplate.query(query.toString(), new RideRowMapper());
	}
	
	public Ride getRideById(String id) {
		StringBuilder query = new StringBuilder("SELECT r.*, u.name user_name FROM heybuddy.rides r INNER JOIN heybuddy.users u ON r.username = u.username  WHERE ride_id = " + id);
		return jdbcTemplate.queryForObject(query.toString(), new RideRowMapper());
	}

	public List<Ride> searchRide(Ride ride) {
		StringBuilder query = new StringBuilder("Select r.*, u.name user_name from heybuddy.rides r");
